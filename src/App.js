...
const App = () => {
  ...
  const enterBoardWord = (word) => {
    ...
    if (matchCount === 5) {
      status = "WIN";
      handleMessage("VOCÊ VENCEU!")
    }
    else if (rowIndex + 1 === 6) {
      status = "LOST";
      handleMessage("A palvara era: " + boardData.solution.toUpperCase())
    }
    ...
    if (key === "ENTER") {
      if (charArray.length === 5) {
        ...
        if (!wordList[word.charAt(0)].includes(word)) {
          handleError();
          handleMessage("Palavra não encontrada na lista de palavras");
          return;
        }
        enterBoardWord(word);
        setCharArray([]);
      } else {
        handleMessage("Letras insuficientes");
      }
      return;
    }
    if (key === "⌫") {
